...
kernel void r_128_16_8(device int* data0, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  ...
  int gidx0 = gid.x; /* 128 */
  int lidx0 = lid.x; /* 16 */
  int alu0 = max((gidx0+(lidx0<<3)+-119),0);
  int alu1 = max(-8,-alu0);
  *(temp0+lidx0) = -alu1;
...
kernel void r_1024_16_64(device int* data0, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  ...
  int lidx0 = lid.x; /* 16 */
  int alu0 = max((gidx0+(lidx0<<6)+-959),0);
  int alu1 = max(-64,-alu0);
  *(temp0+lidx0) = -alu1;
  threadgroup_barrier(mem_flags::mem_threadgroup);
  if (((bool)(lidx0)!=1)) {
    int acc0 = 0;
...
#include <metal_stdlib>
using namespace metal;
kernel void r_13_16_29_16_1733_3(device float* data0, const device int* data1, const device int* data2, const device float* data3, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int gidx0 = gid.x; /* 29 */
  int gidx1 = gid.y; /* 16 */
  int lidx0 = lid.x; /* 16 */
  int gidx2 = gid.z; /* 13 */
...
#include <metal_stdlib>
using namespace metal;
kernel void r_13_16_16_256_3_4(device float* data0, const device int* data1, const device int* data2, const device float* data3, constant int& start_pos, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int gidx0 = gid.x; /* 16 */
  int lidx0 = lid.x; /* 16 */
  int gidx1 = gid.y; /* 13 */
...
#include <metal_stdlib>
using namespace metal;
kernel void r_312_32_29(device float* data0, const device float* data1, const device float* data2, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int gidx0 = gid.x; /* 312 */
  int lidx0 = lid.x; /* 32 */
...
#include <metal_stdlib>
using namespace metal;
kernel void r_13_256_3(device float* data0, const device float* data1, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
...
#include <metal_stdlib>
using namespace metal;
kernel void r_13_256_3n1(device float* data0, const device float* data1, const device float* data2, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  threadgroup float temp0[256];
  int gidx0 = gid.x; /* 13 */
  int lidx0 = lid.x; /* 256 */
...
#include <metal_stdlib>
using namespace metal;
kernel void E_13_16_16_3(device float* data0, const device float* data1, const device float* data2, const device float* data3, const device float* data4, const device float* data5, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int gidx0 = gid.x; /* 16 */
  int gidx1 = gid.y; /* 13 */
  int lidx0 = lid.x; /* 16 */
  int alu0 = (gidx0*48);
...
#include <metal_stdlib>
using namespace metal;
kernel void r_13_48_16_192_3_4(device float* data0, const device float* data1, const device float* data2, const device float* data3, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int gidx0 = gid.x; /* 48 */
  int gidx1 = gid.y; /* 13 */
  int lidx0 = lid.x; /* 16 */
  int alu0 = (lidx0*3);
...
#include <metal_stdlib>
using namespace metal;
kernel void E_13_48_16_2(device float* data0, const device float* data1, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int gidx0 = gid.x; /* 48 */
  int gidx1 = gid.y; /* 13 */
  int lidx0 = lid.x; /* 16 */
  int alu0 = (gidx0<<4);
...
#include <metal_stdlib>
using namespace metal;
kernel void r_13_13_4_16_3_4(device float* data0, const device float* data1, const device float* data2, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int gidx0 = gid.x; /* 13 */
  int gidx1 = gid.y; /* 13 */
  int lidx0 = lid.x; /* 4 */
  int alu0 = (lidx0*192);
...
#include <metal_stdlib>
using namespace metal;
kernel void r_39_4_13(device float* data0, const device float* data1, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int gidx0 = gid.x; /* 39 */
  int lidx0 = lid.x; /* 4 */
  ...
  float alu1 = max(val12,val0);
  float alu2 = max(alu1,val1);
  float alu3 = max(alu2,val2);
  float alu4 = max(alu3,val3);
  float alu5 = max(alu4,val4);
  float alu6 = max(alu5,val5);
  float alu7 = max(alu6,val6);
  float alu8 = max(alu7,val7);
  float alu9 = max(alu8,val8);
  float alu10 = max(alu9,val9);
  float alu11 = max(alu10,val10);
  float alu12 = max(alu11,val11);
  *(data0+(gidx0<<2)+lidx0) = alu12;
}
#include <metal_stdlib>
using namespace metal;
kernel void r_39_4_13n1(device float* data0, const device float* data1, const device float* data2, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int gidx0 = gid.x; /* 39 */
  int lidx0 = lid.x; /* 4 */
  int alu0 = ((gidx0*52)+(lidx0*13));
...
#include <metal_stdlib>
using namespace metal;
kernel void E_13_13_4_3(device float* data0, const device float* data1, const device float* data2, const device float* data3, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int gidx0 = gid.x; /* 13 */
  int gidx1 = gid.y; /* 13 */
  int lidx0 = lid.x; /* 4 */
...
#include <metal_stdlib>
using namespace metal;
kernel void r_3_13_4_16_4_13(device float* data0, const device float* data1, const device float* data2, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int gidx0 = gid.x; /* 13 */
  int gidx1 = gid.y; /* 3 */
  int lidx0 = lid.x; /* 4 */
  int lidx1 = lid.y; /* 16 */
...
#include <metal_stdlib>
using namespace metal;
kernel void r_13_16_16_192_3_4(device float* data0, const device float* data1, const device float* data2, const device float* data3, const device float* data4, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int gidx0 = gid.x; /* 16 */
  int gidx1 = gid.y; /* 13 */
  int lidx0 = lid.x; /* 16 */
  int alu0 = (lidx0*3);
...
#include <metal_stdlib>
using namespace metal;
kernel void r_13_64_16_192_3_4(device float* data0, const device float* data1, const device float* data2, const device float* data3, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int gidx0 = gid.x; /* 64 */
  int gidx1 = gid.y; /* 13 */
  int lidx0 = lid.x; /* 16 */
  int alu0 = (lidx0*3);
  ...
  *(data0+alu3+1) = ((1/(exp2(((alu5+(alu5*alu5*alu5*0.044715f))*-2.302208185195923f))+1.0f))*alu5);
  float val14 = *(data3+alu2+2);
  float alu6 = (acc2+val14);
  *(data0+alu3+2) = ((1/(exp2(((alu6+(alu6*alu6*alu6*0.044715f))*-2.302208185195923f))+1.0f))*alu6);
  float val15 = *(data3+alu2);
  float alu7 = (acc0+val15);
  *(data0+alu3) = ((1/(exp2(((alu7+(alu7*alu7*alu7*0.044715f))*-2.302208185195923f))+1.0f))*alu7);
}
#include <metal_stdlib>
using namespace metal;
kernel void r_13_16_16_768_3_4(device float* data0, const device float* data1, const device float* data2, const device float* data3, const device float* data4, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int gidx0 = gid.x; /* 16 */
  int gidx1 = gid.y; /* 13 */
  int lidx0 = lid.x; /* 16 */
  int alu0 = (lidx0*3);
...
#include <metal_stdlib>
using namespace metal;
kernel void E_(device unsigned int* data0, const device unsigned int* data1, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  unsigned int val0 = *(data1+0);
  *(data0+0) = (val0+786432u);
}
#include <metal_stdlib>
using namespace metal;
kernel void E_n1(device unsigned int* data0, const device unsigned int* data1, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  unsigned int val0 = *(data1+0);
  *(data0+0) = (val0+1769472u);
}
#include <metal_stdlib>
using namespace metal;
kernel void E_n2(device unsigned int* data0, const device unsigned int* data1, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  unsigned int val0 = *(data1+0);
  *(data0+0) = (val0+2304u);
}
#include <metal_stdlib>
using namespace metal;
kernel void E_n3(device unsigned int* data0, const device unsigned int* data1, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  unsigned int val0 = *(data1+0);
  *(data0+0) = (val0+589824u);
}
#include <metal_stdlib>
using namespace metal;
kernel void E_n4(device unsigned int* data0, const device unsigned int* data1, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  unsigned int val0 = *(data1+0);
  *(data0+0) = (val0+768u);
}
#include <metal_stdlib>
using namespace metal;
kernel void E_n5(device unsigned int* data0, const device unsigned int* data1, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  unsigned int val0 = *(data1+0);
  *(data0+0) = (val0+2359296u);
}
#include <metal_stdlib>
using namespace metal;
kernel void E_n6(device unsigned int* data0, const device unsigned int* data1, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  unsigned int val0 = *(data1+0);
  *(data0+0) = (val0+3072u);
}
#include <metal_stdlib>
using namespace metal;
kernel void r_13_50257_192_4(device float* data0, const device float* data1, const device float* data2, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int gidx0 = gid.x; /* 50257 */
  int gidx1 = gid.y; /* 13 */
  float acc0 = 0.0f;
  for (int ridx0 = 0; ridx0 < 192; ridx0++) {
...
#include <metal_stdlib>
using namespace metal;
kernel void r_29_1733(device float* data0, const device float* data1, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int gidx0 = gid.x; /* 29 */
  float acc0 = -INFINITY;
  for (int ridx0 = 0; ridx0 < 1733; ridx0++) {
    float val0 = *(data1+(gidx0*1733)+ridx0+603084);
    float alu0 = max(acc0,(val0*1.25f));
    acc0 = alu0;
  }
  *(data0+gidx0) = acc0;
}
#include <metal_stdlib>
using namespace metal;
kernel void r_29(device float* data0, const device float* data1, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  float val0 = *(data1+28);
  float4 val1 = *((device float4*)(data1+0));
  float alu0 = max(val1.x,val1.y);
  float alu1 = max(alu0,val1.z);
  float alu2 = max(alu1,val1.w);
  float4 val2 = *((device float4*)(data1+4));
  float alu3 = max(alu2,val2.x);
  float alu4 = max(alu3,val2.y);
  float alu5 = max(alu4,val2.z);
  float alu6 = max(alu5,val2.w);
  float4 val3 = *((device float4*)(data1+8));
  float alu7 = max(alu6,val3.x);
  float alu8 = max(alu7,val3.y);
  float alu9 = max(alu8,val3.z);
  float alu10 = max(alu9,val3.w);
  float4 val4 = *((device float4*)(data1+12));
  float alu11 = max(alu10,val4.x);
  float alu12 = max(alu11,val4.y);
  float alu13 = max(alu12,val4.z);
  float alu14 = max(alu13,val4.w);
  float4 val5 = *((device float4*)(data1+16));
  float alu15 = max(alu14,val5.x);
  float alu16 = max(alu15,val5.y);
  float alu17 = max(alu16,val5.z);
  float alu18 = max(alu17,val5.w);
  float4 val6 = *((device float4*)(data1+20));
  float alu19 = max(alu18,val6.x);
  float alu20 = max(alu19,val6.y);
  float alu21 = max(alu20,val6.z);
  float alu22 = max(alu21,val6.w);
  float4 val7 = *((device float4*)(data1+24));
  float alu23 = max(alu22,val7.x);
  float alu24 = max(alu23,val7.y);
  float alu25 = max(alu24,val7.z);
  float alu26 = max(alu25,val7.w);
  float alu27 = max(alu26,val0);
  *(data0+0) = alu27;
}
#include <metal_stdlib>
using namespace metal;
kernel void r_29_1733n1(device float* data0, const device float* data1, const device float* data2, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int gidx0 = gid.x; /* 29 */
  float val0 = *(data2+0);
  float acc0 = 0.0f;
  for (int ridx0 = 0; ridx0 < 1733; ridx0++) {
...
#include <metal_stdlib>
using namespace metal;
kernel void r_29n1(device float* data0, const device float* data1, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  float val0 = *(data1+28);
  float4 val1 = *((device float4*)(data1+0));
  float4 val2 = *((device float4*)(data1+4));
  float4 val3 = *((device float4*)(data1+8));
...
#include <metal_stdlib>
using namespace metal;
kernel void E_50257(device float* data0, const device float* data1, const device float* data2, const device float* data3, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int gidx0 = gid.x; /* 50257 */
...
#include <metal_stdlib>
using namespace metal;
kernel void r_197_16_16_64_4(device float* data0, const device float* data1, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int gidx0 = gid.x; /* 16 */
  int gidx1 = gid.y; /* 197 */
  int lidx0 = lid.x; /* 16 */
  int alu0 = (gidx0<<4);
...
#include <metal_stdlib>
using namespace metal;
kernel void r_197_197(device float* data0, const device float* data1, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int gidx0 = gid.x; /* 197 */
  float acc0 = 0.0f;
...
#include <metal_stdlib>
using namespace metal;
kernel void E_n7(device unsigned int* data0, const device unsigned int* data1, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  unsigned int val0 = *(data1+0);
  *(data0+0) = (val0+38597376u);
}
#include <metal_stdlib>
using namespace metal;
kernel void E_n8(device unsigned int* data0, const device unsigned int* data1, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  unsigned int val0 = *(data1+0);
  *(data0+0) = (val0+1u);
}
#include <metal_stdlib>
using namespace metal;
kernel void E_n9(device unsigned int* data0, const device unsigned int* data1, const device unsigned long* data2, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  unsigned int val0 = *(data1+0);
...
#include <metal_stdlib>
using namespace metal;
kernel void E_n10(device unsigned int* data0, const device unsigned int* data1, const device unsigned long* data2, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  float precast0 = 1.0f;
  unsigned int val0 = *(data1+0);
  unsigned long alu0 = (((unsigned long)((val0+1u))<<32)|(unsigned long)(val0));
  unsigned int cast0 = (unsigned int)(((alu0>>32)&4294967295ull));
  ...
  unsigned int alu44 = ((alu43+10u)^((alu42<<26)+(alu42>>6)));
  unsigned int alu45 = (alu43+alu44);
  *(data0+0) = (((unsigned int)(((((unsigned long)((((alu45+10u)^((alu44<<6)+(alu44>>26)))+cast1+5u))<<32)|(unsigned long)((alu45+alu3+10u)))&4294967295ull))>>9)|as_type<unsigned int>(precast0));
}
#include <metal_stdlib>
using namespace metal;
kernel void E_n11(device float* data0, const device float* data1, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  float val0 = *(data1+0);
  *(data0+0) = (val0+-1.0f);
}
#include <metal_stdlib>
using namespace metal;
kernel void r_29_1733n2(device int* data0, const device float* data1, const device float* data2, const device float* data3, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int gidx0 = gid.x; /* 29 */
  float val0 = *(data1+0);
...
#include <metal_stdlib>
using namespace metal;
kernel void r_29n2(device int* data0, const device int* data1, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int val0 = *(data1+0);
  int val1 = *(data1+1);
  int val2 = *(data1+2);
  int val3 = *(data1+3);
...
#include <metal_stdlib>
using namespace metal;
kernel void r_768_16_64(device float* data0, const device float* data1, const device int* data2, const device int* data3, const device float* data4, constant int& start_pos, constant int& tokens, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
...
#include <metal_stdlib>
using namespace metal;
kernel void r_256_3(device float* data0, const device float* data1, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  threadgroup float temp0[256];
  int lidx0 = lid.x; /* 256 */
  float acc0 = 0.0f;
  for (int ridx0 = 0; ridx0 < 3; ridx0++) {
...
#include <metal_stdlib>
using namespace metal;
kernel void r_256_3n1(device float* data0, const device float* data1, const device float* data2, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  threadgroup float temp0[256];
  int lidx0 = lid.x; /* 256 */
  float val0 = *(data2+0);
  float acc0 = 0.0f;
...
#include <metal_stdlib>
using namespace metal;
kernel void E_6_32_4(device float* data0, const device float* data1, const device float* data2, const device float* data3, const device float* data4, const device float* data5, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int gidx0 = gid.x; /* 6 */
  int lidx0 = lid.x; /* 32 */
...
#include <metal_stdlib>
using namespace metal;
kernel void r_144_4_8_96_4(device float* data0, const device float* data1, const device float* data2, const device float* data3, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int lidx0 = lid.x; /* 4 */
...
#include <metal_stdlib>
using namespace metal;
kernel void E_24_32_2(device float* data0, const device float* data1, constant int& start_pos, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int gidx0 = gid.x; /* 24 */
  int lidx0 = lid.x; /* 32 */
  int alu0 = (gidx0<<5);
...
#include <metal_stdlib>
using namespace metal;
kernel void r_28start_pos2B129_4_16_3_4(device float* data0, const device float* data1, const device float* data2, constant int& start_pos, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
...
#include <metal_stdlib>
using namespace metal;
kernel void r_3_4_28start_pos2B129(device float* data0, const device float* data1, constant int& start_pos, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int gidx0 = gid.x; /* 3 */
  int lidx0 = lid.x; /* 4 */
...
#include <metal_stdlib>
using namespace metal;
kernel void r_3_4_28start_pos2B129n1(device float* data0, const device float* data1, const device float* data2, constant int& start_pos, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int gidx0 = gid.x; /* 3 */
  int lidx0 = lid.x; /* 4 */
  int alu0 = (start_pos+1);
  int alu1 = ((gidx0<<2)+lidx0);
...
#include <metal_stdlib>
using namespace metal;
kernel void E_28start_pos2B129_4_3(device float* data0, const device float* data1, const device float* data2, const device float* data3, constant int& start_pos, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int gidx0 = gid.x; /* UOp(UOps.ALU, dtypes.int, arg=BinaryOps.ADD, src=(
  UOp(UOps.DEFINE_VAR, dtypes.int, arg=('start_pos', 1, 128), src=()),
  UOp(UOps.CONST, dtypes.int, arg=1, src=()),)) */
  int lidx0 = lid.x; /* 4 */
...
#include <metal_stdlib>
using namespace metal;
kernel void r_3_4_4_16_28start_pos2B129(device float* data0, const device float* data1, const device float* data2, constant int& start_pos, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int gidx0 = gid.x; /* 4 */
  int gidx1 = gid.y; /* 3 */
  int lidx0 = lid.x; /* 4 */
  int lidx1 = lid.y; /* 16 */
...
#include <metal_stdlib>
using namespace metal;
kernel void r_48_4_8_96_4(device float* data0, const device float* data1, const device float* data2, const device float* data3, const device float* data4, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int lidx0 = lid.x; /* 4 */
  threadgroup float temp0[128];
  int gidx0 = gid.x; /* 48 */
  int lidx1 = lid.y; /* 8 */
...
#include <metal_stdlib>
using namespace metal;
kernel void r_192_4_8_96_4(device float* data0, const device float* data1, const device float* data2, const device float* data3, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int lidx0 = lid.x; /* 4 */
  threadgroup float temp0[128];
  int gidx0 = gid.x; /* 192 */
  int lidx1 = lid.y; /* 8 */
  ...
  *((threadgroup float4*)(temp0+alu1+(lidx1<<2))) = float4(acc0,acc1,acc2,acc3);
  ...
  if (((bool)(lidx1)!=1)) {
    ...
    *(data0+alu2) = ((1/(exp2(((alu7+(alu7*alu7*alu7*0.044715f))*-2.302208185195923f))+1.0f))*alu7);
    *(data0+alu3) = ((1/(exp2(((alu8+(alu8*alu8*alu8*0.044715f))*-2.302208185195923f))+1.0f))*alu8);
    *(data0+alu4) = ((1/(exp2(((alu9+(alu9*alu9*alu9*0.044715f))*-2.302208185195923f))+1.0f))*alu9);
    *(data0+alu0) = ((1/(exp2(((alu6+(alu6*alu6*alu6*0.044715f))*-2.302208185195923f))+1.0f))*alu6);
  }
}
#include <metal_stdlib>
using namespace metal;
kernel void r_48_4_8_384_4(device float* data0, const device float* data1, const device float* data2, const device float* data3, const device float* data4, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int lidx0 = lid.x; /* 4 */
  threadgroup float temp0[128];
  int gidx0 = gid.x; /* 48 */
  int lidx1 = lid.y; /* 8 */
...
#include <metal_stdlib>
using namespace metal;
kernel void E_n12(device unsigned int* data0, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  unsigned int val0 = *(data0+0);
  *(data0+0) = (val0+1u);
}
#include <metal_stdlib>
using namespace metal;
kernel void r_50257_192_4(device float* data0, const device float* data1, const device float* data2, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int gidx0 = gid.x; /* 50257 */
  float acc0 = 0.0f;
  for (int ridx0 = 0; ridx0 < 192; ridx0++) {
    int alu0 = (ridx0<<2);
...
#include <metal_stdlib>
using namespace metal;
kernel void r_29_1733n3(device float* data0, const device float* data1, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int gidx0 = gid.x; /* 29 */
  float acc0 = -INFINITY;
  for (int ridx0 = 0; ridx0 < 1733; ridx0++) {
    float val0 = *(data1+(gidx0*1733)+ridx0);
    float alu0 = max(acc0,val0);
    acc0 = alu0;
  }
  *(data0+gidx0) = acc0;
}
#include <metal_stdlib>
using namespace metal;
kernel void r_29_1733n4(device float* data0, const device float* data1, const device float* data2, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int gidx0 = gid.x; /* 29 */
  float val0 = *(data2+0);
  float acc0 = 0.0f;
  for (int ridx0 = 0; ridx0 < 1733; ridx0++) {
...
#include <metal_stdlib>
using namespace metal;
kernel void E_50257n1(device float* data0, const device float* data1, const device float* data2, const device float* data3, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  int gidx0 = gid.x; /* 50257 */
